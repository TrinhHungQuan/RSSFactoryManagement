...
const RoleAdding = () => {
  ...
  // Fetch all permissions on first load
  const fetchAllPermissions = async () => {
    try {
      const token =
        localStorage.getItem("token") || sessionStorage.getItem("token");

      const response = await axios.get(API_ENDPOINTS.getPermissions, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      const fetchedCompanies: Permission[] = response.data.result.map(
      ...
      setAllPermissions(fetchedCompanies);
    } catch (err) {
    ...
    <>
      ...
      <div className="mx-6 mt-6">
        ...
        <div className="flex justify-start items-center gap-3 mb-4">
          ...
          <h1 className="text-xl font-semibold">Add New Role Admin</h1>
        </div>
